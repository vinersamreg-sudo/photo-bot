...
Current commercial gate (24.07.2026): site may be submitted for Robokassa moderation. Code and Nginx implement a fail-closed POST ResultURL transport; code and cabinet use SHA-256. An owner-only sandbox reached the provider, but Robokassa rejected the invoice before payment with error 33 because Pixora rendered a UTC deadline as an offset-free `ExpirationDate`, which Robokassa interpreted as Moscow time. The code now converts timezone-aware deadlines to UTC+03 before rendering and has a regression test, but a post-fix sandbox payment has not yet been authorized or executed. No ResultURL callback, package grant, receipt or refund occurred. Production is back to observe-only/payment fail-closed and pilot limit 0. Sandbox evidence, real payment/refund/reconciliation and final production approval still do not exist; `launch-status` keeps `public_launch_ready=false`.
