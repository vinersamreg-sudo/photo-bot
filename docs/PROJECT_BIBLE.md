...
Demo MVP закрепляет одну исходную фотографию за одним пользователем MAX, создаёт до пяти успешно доставленных уменьшенных результатов с крупным watermark «ОБРАЗЕЦ» и хранит оригиналы приватно. Пользователь платит за разблокировку конкретного уже увиденного результата. Тонкий MAX transport и restart-safe application flow реализованы и проверены fake-контрактами, но live MAX ещё не подтверждён и эквайринг не подключён.
...
Production Environment GitHub использует `HETZNER_HOST`, `HETZNER_USER`, `HETZNER_SSH_PORT`, `HETZNER_SSH_PRIVATE_KEY`, `OPENAI_API_KEY`. Для MAX после фактического создания/подтверждения бота добавляется `MAX_BOT_TOKEN`, а для Webhook — `MAX_WEBHOOK_SECRET`. Сейчас MAX secrets отсутствуют; это явный live-блокер.
