...
Use: фотография, изменение, обработка, вариант, демо, водяной знак, работа, версия, оригинал, пакет доступа Ravuna.
...
- Start: `✨ Ravuna` + `Отправьте фотографию и напишите, что хотите изменить.` + short consent notice.
...
- Before redirect: `Пакет доступа Ravuna — 49 ₽.` + `После оплаты вам начисляется пакет доступа Ravuna. В пакет входят две обработки и один оригинал. Пакет начисляется сразу после подтверждения оплаты.` + link button `Оплатить 49 ₽ в Robokassa`.
...
- After confirmation: `Пакет доступа Ravuna начислен.` and show balances as `обработок` and `оригиналов`, never `генераций`.
- The public product name and Robokassa `Description` are «Пакет доступа Ravuna». The separately controlled fiscal Receipt item is not user-interface copy.
