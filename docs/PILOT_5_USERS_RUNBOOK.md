...
Пилот подготовлен, но не включён. Он проверяет генерацию/UX/поддержку и может проходить с платежами полностью выключенными.

## Stage 0 — безопасное состояние

`MAX_POLL_OBSERVE_ONLY=true`, `PILOT_USER_LIMIT=0`, payment/webhook/refunds false. Выполнить `launch-status --strict`, backup/restore/off-site, SQLite quick_check, processing=0, orphan=0.
...
2. Внести ровно пять ID в GitHub Environment secret `MAX_PILOT_USER_IDS` в нужном порядке. Не коммитить и не выводить их.
...
Остановить новые генерации: немедленно выполнить обычный push/manual deploy с handlers disabled и `pilot_user_limit=0`; убедиться в `MAX_POLL_OBSERVE_ONLY=true`. Уже начатую операцию не убивать без проверки целостности; дождаться timeout/recovery, затем processing=0.
...
> Вы приглашены в закрытый тест Pixora. Используйте свою фотографию, на которую у вас есть права. Доступны две успешно доставленные обработки с водяным знаком на одной или разных фотографиях; результат создаётся автоматически и может отличаться от ожиданий. «Пакет Pixora: 2 варианта обработки и 1 оригинал — 49 ₽» сейчас не включён. Оставьте оценку кнопками после результата, удалите работу через «Мои работы», а при проблеме напишите на viner-89@mail.ru без отправки банковских данных.
