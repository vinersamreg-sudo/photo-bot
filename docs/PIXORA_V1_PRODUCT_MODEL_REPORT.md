...
21. Рабочий Receipt item намеренно остаётся «Пакет Pixora: 2 варианта обработки и 1 оригинал»; `tax=none`, а точные `payment_method`/`payment_object` для Робочеков СМЗ требуют подтверждения Robokassa до sandbox. Публичное название продукта не вычисляется из Receipt.
