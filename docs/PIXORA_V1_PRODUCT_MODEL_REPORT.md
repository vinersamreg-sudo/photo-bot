# Pixora v1 — отчёт о постоянной модели 2+1
...
2. Новый MAX user ID теперь получает ровно две generation credits один раз за всю жизнь identity.
...
5. Пакет 49 ₽: `continuation_pack_2_plus_1`, +2 generation credits и +1 unlock entitlement.
...
19. Оферта описывает две бесплатные успешные обработки, единый пакет 49 ₽, отдельный выбор original и правила возврата.
20. Главная, FAQ и structured data обещают «ещё 2 варианта + 1 оригинал» и не обещают подписку.
21. Рабочий Receipt item: «Пакет Pixora: 2 варианта обработки и 1 оригинал»; `tax=none`, а точные `payment_method`/`payment_object` для Робочеков СМЗ требуют подтверждения Robokassa до sandbox.
